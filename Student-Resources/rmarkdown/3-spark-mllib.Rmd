---
title: "Machine Learning Pipelines with Sparklyr and Spark ML"
output: 
  html_notebook:
    toc: true
    toc_depth: 2
...
# Training Machine Learning Algorithms with Sparklyr
...
dtree_conf_df <- sdf_conf_df() %>% collect

library(tidyr)
conf_spread <- . %>% spread(key = "good_tip", value  = "n")

dtree_conf <- dtree_conf_df %>% conf_spread
...
We can use the SparkML evaluation module to calculate evaluation metrics directly from our scored data.
...
sprintf("The AUC for the decision tree is %s", dtree_auc)
sprintf("The AUC for the random forest is %s", dforest_auc)
sprintf("The accuracy of the random forest is %s", dforest_accuracy)
